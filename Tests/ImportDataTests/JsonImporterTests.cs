...
namespace BigHW1.Tests.ImportDataTests
{
    ...
    {
        [Fact]
        public void Import_ValidJsonFile_ParsesAllData()
        {
            // Arrange
            var currentDir = Directory.GetCurrentDirectory();
            var projectDir = Directory.GetParent(currentDir)?.Parent?.Parent?.FullName
                             ?? currentDir;
            // Путь к файлу export.json в папке TestData
            string filePath = Path.Combine(projectDir, "TestData", "import.json");
            ...
            Assert.Equal(2025, op2.Date.Year);
            Assert.Equal(3, op2.Date.Month);
            Assert.Equal(17, op2.Date.Day);
        }
